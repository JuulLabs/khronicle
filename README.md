...
A `tag` is required, and an optional `throwable` may be provided.
...
Log.debug("Tag") { "This is not called." }
Log.warn("Tag") { "This still gets called." }
```
...
Log.debug("Tag") { "This block is evaluated, but does not get printed to the console." }
Log.warn("Tag") { metadata ->
    metadata[Sensitivity] = Sensitivity.NotSensitive
